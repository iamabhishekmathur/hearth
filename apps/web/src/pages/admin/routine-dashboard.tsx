  ...
  return (
    <div className="rounded-lg border border-hearth-border p-4">
      <div className="mb-2 flex items-center justify-between">
        <h4 className="truncate text-sm font-semibold text-hearth-text">{analytic.routineName}</h4>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div>
          <span className="text-hearth-text-faint">Runs</span>
          <p className="font-medium text-hearth-text">{analytic.totalRuns}</p>
        </div>
        <div>
          <span className="text-hearth-text-faint">Avg Duration</span>
        ...
        <div>
          <span className="text-hearth-text-faint">Tokens</span>
          <p className="font-medium text-hearth-text">{analytic.totalTokens.toLocaleString()}</p>
        </div>
        <div>
          <span className="text-hearth-text-faint">Last Run</span>
          <p className="font-medium text-hearth-text">
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="border-b border-hearth-border px-6 py-4">
        <h1 className="text-xl font-bold text-hearth-text">Routine Health</h1>
        <p className="mt-0.5 text-sm text-hearth-text-muted">Organization-wide routine monitoring</p>
      </div>

      <div className="flex-1 overflow-y-auto p-6">
        {loading ? (
          <div className="flex items-center justify-center py-12">
            <div className="h-8 w-8 animate-spin rounded-full border-2 border-hearth-border border-t-hearth-accent" />
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-lg border border-hearth-border bg-hearth-card p-4">
                <p className="text-xs font-medium uppercase tracking-wide text-hearth-text-faint">Total Runs</p>
                <p className="mt-1 text-2xl font-bold text-hearth-text">{totalRuns.toLocaleString()}</p>
              </div>
              <div className="rounded-lg border border-hearth-border bg-hearth-card p-4">
                <p className="text-xs font-medium uppercase tracking-wide text-hearth-text-faint">Avg Success Rate</p>
                <p className="mt-1 text-2xl font-bold text-hearth-text">{avgSuccessRate.toFixed(1)}%</p>
              </div>
              <div className="rounded-lg border border-hearth-border bg-hearth-card p-4">
                <p className="text-xs font-medium uppercase tracking-wide text-hearth-text-faint">Total Tokens</p>
                <p className="mt-1 text-2xl font-bold text-hearth-text">{totalTokens.toLocaleString()}</p>
              </div>
            </div>

            {/* Routine health cards */}
            <div>
              <h2 className="mb-3 text-sm font-semibold text-hearth-text">Routine Performance</h2>
              {analytics.length === 0 ? (
                <p className="text-sm text-hearth-text-faint">No routine data available.</p>
              ) : (
            ...
            {/* Health alerts */}
            <div>
              <h2 className="mb-3 text-sm font-semibold text-hearth-text">Health Alerts</h2>

              {alerts.length > 0 && (
                <div className="mb-3 space-y-2">
                  {alerts.map((alert) => (
                    <div key={alert.id} className="flex items-center justify-between rounded-lg border border-hearth-border px-4 py-2.5">
                      <div>
                        <span className="text-sm font-medium text-hearth-text">
                          {alert.alertType.replace(/_/g, ' ')}
                        </span>
                        <span className="ml-2 text-xs text-hearth-text-faint">
              ...
              <div className="flex items-end gap-2">
                <div className="flex-1">
                  <select
                    ...
                    className="block w-full rounded-lg border border-hearth-border-strong px-3 py-2 text-sm"
                  >
                ...
                <select
                  ...
                  className="rounded-lg border border-hearth-border-strong px-3 py-2 text-sm"
                >
