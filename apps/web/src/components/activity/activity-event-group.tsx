...
export function ActivityEventGroup({ action, events }: ActivityEventGroupProps) {
  ...
  return (
    <button
      ...
      className="flex w-full items-center gap-3 rounded-lg border border-gray-100 bg-white p-3 text-left hover:bg-gray-50"
    >
      {/* Stacked avatars */}
      <div className="flex -space-x-2">
        {displayUsers.map((name) => (
          <div
            key={name}
            className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-white bg-gray-100 text-xs font-medium text-gray-700"
          >
